      ...
      const html = res.data.map(i => {
        ...
        return \
          <div class="pedido-card" style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <div>
              <button class="btn btn-secondary btn-small" onclick="editarInsumo(\)">?? Editar</button>
              <button class="btn btn-secondary btn-small" style="color:var(--danger)" onclick="eliminarInsumo(\)">???</button>
            </div>
          </div>
        \;
      }).join('');
    ...
    if (res.ok) {
      ...
      if (inventarioState.tabActual === 'productos') {
        const html = res.data.map(p => \
          <div class="pedido-card" style="display:flex; justify-content:space-between; align-items:center;">
            <div>
              <strong>\</strong> <span class="badge" style="background:var(--surface2)">\</span>
              <div style="color:var(--text2); font-size:12px; margin-top:4px;">
                Precio: $\ | Stock directo: \
              </div>
            </div>
            <div>
              <button class="btn btn-secondary btn-small" onclick="editarProducto(\)">?? Editar</button>
              <button class="btn btn-secondary btn-small" style="color:var(--danger)" onclick="eliminarProducto(\)">???</button>
            </div>
          </div>
        \).join('');
        ...
        const listaHtml = res.data.map(p => \
          <li onclick="seleccionarProductoReceta(\)" id="receta-prod-\">\</li>
        \).join('');
...
function agregarFilaReceta(data = null) {
  ...
  const insumosOptions = inventarioState.insumos.map(i => \<option value="\" \>\ (\)</option>\).join('');
  ...
  div.innerHTML = \
    ...
    <input type="number" class="edit-input input-cant-insumo" step="0.01" value="\" placeholder="Cant.">
