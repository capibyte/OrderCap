...
function startServer(db, mainWindow) {
  ...
  // ── Middleware ─────────────────────────────────────────────────────────────
  // Habilitar CORS para recibir llamadas de cualquier origen local (ej: Docker host.docker.internal)
  app.use(cors());
  ...
  app.post('/api/pedidos', (req, res) => {
    try {
      ...
      if (!pedidoObj.cliente_nombre) {
        return res.status(400).json({ ok: false, error: 'El campo cliente_nombre es requerido.' });
      }
      if (!pedidoObj.productos) {
        return res.status(400).json({ ok: false, error: 'El listado de productos es requerido.' });
      }
      ...
      // Obtener el total enviado por el cliente/n8n
      ...
      if (Math.abs(totalEnviado - totalCalculado) > 0.01) {
        console.log(`[Server] Discrepancia detectada: enviado=$${totalEnviado}, calculado=$${totalCalculado}. Forzando flag_alerta = 1`);
        flag_alerta = 1;
      }
      ...
      const executeTx = db.transaction(() => {
        ...
        // 2. Descontar stock (idéntica lógica robusta del main process)
        for (const p of productosObj) {
      ...
      if (liveWindow && !liveWindow.webContents.isDestroyed()) {
        console.log(`[Server] Enviando pedido ${savedPedido.numero_pedido} al Renderer via IPC. flag_alerta=${flag_alerta === 1}`);
